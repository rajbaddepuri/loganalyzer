...
def insert_log_path(log_path):
    try:
        log_path = str(log_path).replace("\\","_")
        with connection.cursor() as cursor:
...
def process_file_data(lines, updated_lines=False):
    if lines is None:
        return False
    for i in lines:
        value = str(i).split(" ")
...
def get_group_by_ip_address():
    record = None
    try:
        with connection.cursor() as cursor:
            sql = "SELECT lp_adrees, count(lp_adrees) FROM loganalyzerapp_log_fields GROUP BY lp_adrees;"
            cursor.execute(sql)
            record = cursor.fetchall()
    except Exception as e:
        log.warning(e)
